...
function SupplierInfo({
    ...
    onProfileClick 
     ...
     const flagUrl = useCountryFlag(location);
  ...
  return (
      ...
      <div className="flex gap-3 md:gap-1.5 md:flex-col text-sm text-gray-700 md:py-5 pt-2 space-y-1 md:mb-3">
        <p className='flex items-center gap-3'>
            {flagUrl && <img src={flagUrl} alt="Flag" className='w-6 h-4 rounded-xs' />}
             ...
             <span className="text-lg md:text-base hidden md:block">{location}</span> {/* Desktop: full */}
        </p>
            {verified ? (
    ...
    <button className='hidden  w-full md:flex text-blue-500 items-center gap-3 
    hover:to-blue-700 justify-center cursor-pointer'>
     <FiHeart  className='mt-1 text-xl'/> Save for later</button>
</div>
